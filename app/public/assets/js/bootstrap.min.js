...
if (typeof jQuery === 'undefined') throw new Error("Bootstrap's JavaScript requires jQuery"); +function(a) { 'use strict'; const b = a.fn.jquery.split(' ')[0].split('.'); if (b[0] < 2 && b[1] < 9 || b[0] == 1 && b[1] == 9 && b[2] < 1 || b[0] > 3) throw new Error("Bootstrap's JavaScript requires jQuery version 1.9.1 or higher, but lower than version 4"); }(jQuery), +function(a) {
  ...
  'use strict'; function b(b) {
    ...
    const c = this.getItemIndex(b),
      d = a == 'prev' && c === 0 || a == 'next' && c == this.$items.length - 1; if (d && !this.options.wrap) return b; const e = a == 'prev' ? -1 : 1,
  ...
  'use strict'; function b(b) { let c = b.attr('data-target'); c || (c = b.attr('href'), c = c && /#[A-Za-z]/.test(c) && c.replace(/.*(?=#[^\s]*$)/, '')); const d = c && a(c); return d && d.length ? d : b.parent(); } function c(c) {
    ...
    const e = a(this); if (!e.is('.disabled, :disabled')) {
      const f = b(e),
        ...
        'ontouchstart' in document.documentElement && !f.closest('.navbar-nav').length && a(document.createElement('div')).addClass('dropdown-backdrop').insertAfter(a(this))
          .on('click', c); const h = { relatedTarget: this }; if (f.trigger(d = a.Event('show.bs.dropdown', h)), d.isDefaultPrevented()) return; e.trigger('focus').attr('aria-expanded', 'true'), f.toggleClass('open').trigger(a.Event('shown.bs.dropdown', h));
      } return !1;
    }
    ...
    if (/(38|40|27|32)/.test(c.which) && !/input|textarea/i.test(c.target.tagName)) {
      const d = a(this); if (c.preventDefault(), c.stopPropagation(), !d.is('.disabled, :disabled')) {
        const e = b(d),
          g = e.hasClass('open'); if (!g && c.which != 27 || g && c.which == 27) return c.which == 27 && e.find(f).trigger('focus'), d.trigger('click'); const h = ' li:not(.disabled):visible a',
  ...
  'use strict'; function b(b) {
    ...
    const b = a.Event('show.bs.' + this.type); if (this.hasContent() && this.enabled) {
      this.$element.trigger(b); const d = a.contains(this.$element[0].ownerDocument.documentElement, this.$element[0]); if (b.isDefaultPrevented() || !d) return; const e = this,
        ...
        .data('bs.' + this.type, this), this.options.container ? f.appendTo(this.options.container) : f.insertAfter(this.$element), this.$element.trigger('inserted.bs.' + this.type); const k = this.getPosition(),
        l = f[0].offsetWidth,
    ...
    let d = this.tip(),
      ...
      h = parseInt(d.css('margin-left'), 10); isNaN(g) && (g = 0), isNaN(h) && (h = 0), b.top += g, b.left += h, a.offset.setOffset(d[0], a.extend({ using(a) { d.css({ top: Math.round(a.top), left: Math.round(a.left) }); } }, b), 0), d.addClass('in'); const i = d[0].offsetWidth,
    ...
    b = b || this.$element; let c = b[0],
      d = c.tagName == 'BODY',
      e = c.getBoundingClientRect(); e.width == null && (e = a.extend({}, e, { width: e.right - e.left, height: e.bottom - e.top })); const f = window.SVGElement && c instanceof window.SVGElement,
  ...
  'use strict'; function b(b) {
    ...
    const e = this.$target.scrollTop(),
      ...
      g = this.$target.height(); if (c != null && this.affixed == 'top') return e < c && 'top'; if (this.affixed == 'bottom') return c != null ? !(e + this.unpin <= f.top) && 'bottom' : !(e + g <= a - d) && 'bottom'; const h = this.affixed == null,
      i = h ? e : f.top,
